...
function validate(input: CreateRecipeInput): void {
  if (!input.perfumeId) {
    throw new InvalidRecipeDataError("Falta el perfume asociado.");
  }
  if (input.yieldMl <= 0) {
    throw new InvalidRecipeDataError("El rendimiento debe ser mayor a cero.");
  }
  ...
  for (const item of input.items) {
    ...
    if (item.quantity <= 0) {
      throw new InvalidRecipeDataError("La cantidad de cada ítem debe ser mayor a cero.");
    }
  }
}
